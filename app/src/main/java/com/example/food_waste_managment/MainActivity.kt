...
class MainActivity : AppCompatActivity() {
    ...
    val dropDownList = arrayOf("Select User Type","Hotel","User","NGO","Farmer")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusFillter.onItemSelectedListener = object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                ...
                }else if(statusFillter.selectedItemPosition==3)
                {
                    Toast.makeText(applicationContext,statusFillter.selectedItem.toString(), Toast.LENGTH_LONG).show()
                    val intent = Intent(applicationContext,Ngologin::class.java)

                    //call to the NGO  login

                    startActivity(intent)

                }else if(statusFillter.selectedItemPosition==4)
                {
                    Toast.makeText(applicationContext,statusFillter.selectedItem.toString(), Toast.LENGTH_LONG).show()
        ...
        btnlogin.setOnClickListener {
            ...
            else if (!android.util.Patterns.EMAIL_ADDRESS.matcher(edemail.text.toString()).matches() ){
