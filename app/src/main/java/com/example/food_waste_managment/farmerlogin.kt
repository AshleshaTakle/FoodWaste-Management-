...
class farmerlogin : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //got to farmer register activity

        val btnre = findViewById<Button>(R.id.btnregister)
        btnre.setOnClickListener {
            val intent = Intent(applicationContext,farmerregister::class.java)
            ...
            auth.signInWithEmailAndPassword(edemail.text.toString(),edpassword.text.toString())
                .addOnCompleteListener {
                    if(it.isSuccessful)
                    {
                        ...
                        val intent = Intent(applicationContext, farmerdash::class.java)
                        startActivity(intent)
                    }
                    else
                    {
